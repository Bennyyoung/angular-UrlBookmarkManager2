...
export class BookmarksComponent implements OnInit {
  ...
  constructor(private bookmarkService: BookmarksService, private localSt:LocalStorageService) {}

  ngOnInit() {
    this.getUrlPreview('https://node.js');
    // this.bookmarkService.getBookmark();
    this.bookmarks = this.bookmarkService.getBookmark()
  ...
  onSubmit({ value, valid }: { value: Bookmark; valid: boolean }) {
    if (!valid) {
      ...
      this.localSt.store('bookmarks', JSON.stringify(value))
  ...
  addFavourite(event: any, bookmark: Bookmark) {
    this.bookmarks = this.bookmarks.map((data) => {
      if (data.name == bookmark.name)
      return data;
    });
    console.log(this.bookmarks.indexOf(event, 1));
    this.bookmarks.indexOf(event)
  }
